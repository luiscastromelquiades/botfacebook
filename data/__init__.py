...
from structs import quick_replie_location
from structs import template_message_generic
from structs import element_template_message
from structs import button_element_template_message_url
from structs import button_element_template_message_postback
...
def create_quick_replie_location(user, data):
    return quick_replie_location(user['user_id'], data['content'])
...
def create_element_template_message(data):
    buttons = [ create_button_element_template_message(button) for button in data['buttons'] ]
    return element_template_message(data['title'], data['subtitle'], data['item_url'], data['image_url'], buttons)

def create_button_element_template_message(data):
    if data['type'] == 'web_url':
        return button_element_template_message_url(data['title'], data['url'])
    else:
        return button_element_template_message_postback(data['title'], data['payload'])

def create_image_message(user, data):
    url = data.get('url')
    return multimedia_message(user['user_id'], 'image', url )
